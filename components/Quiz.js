...
import { setAsCompleted } from '../actions/index'
...
class Quiz extends Component{
    ...
    answer = (correct, deck, index) => {
        ...
        if(deck.questions.length - 1 === index){ // determine if deck is completed
            this.props.handleSetAsCompleted(deck.title)

            SetNotification() // postphone when a deck is completed for that day
            console.log('Notification is postphoned tomorrow cause a deck is completed')
        }
    ...
    score = (deckTitle) => {
        ...
        this.setState({numberOfSucceed: 0})
...
const mapDispatchToProps = dispatch => {
    return {
        handleSetAsCompleted: (deckTitle) => dispatch(setAsCompleted(deckTitle))
    }
}
